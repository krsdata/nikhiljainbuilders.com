	...
	class Winsome_Latest_News_Widget extends WP_Widget {
	    ...
	    function form( $instance ) {
	        ...
	        <p>
	          ...
	          <input class="widefat1" id="<?php echo esc_attr( $this->get_field_id( 'post_number' ) ); ?>" name="<?php echo  esc_attr( $this->get_field_name( 'post_number' ) ); ?>" type="number" value="<?php echo esc_attr( $instance['post_number'] ); ?>" min="1" />
	        </p>
	        <p>
	...
	class Winsome_Services_Widget extends WP_Widget {
		...
		function update( $new_instance, $old_instance ) {
			...
			for ( $i = 1; $i <= $item_number; $i++ ) {
				...
				$instance["item_icon_$i"] = sanitize_textfield( $new_instance["item_icon_$i"] );
			}
